...
const announcementTypes = [
  { value: "info", label: "Thông tin", icon: Info, bgColor: "bg-blue-50", borderColor: "border-blue-200", textColor: "text-blue-700", iconBg: "bg-blue-100" },
  { value: "success", label: "Thành công", icon: Check, bgColor: "bg-green-50", borderColor: "border-green-200", textColor: "text-green-700", iconBg: "bg-green-100" },
  { value: "warning", label: "Cảnh báo", icon: AlertTriangle, bgColor: "bg-yellow-50", borderColor: "border-yellow-200", textColor: "text-yellow-700", iconBg: "bg-yellow-100" },
  { value: "celebration", label: "Chúc mừng", icon: PartyPopper, bgColor: "bg-pink-50", borderColor: "border-pink-200", textColor: "text-pink-700", iconBg: "bg-pink-100" },
];
...
export default function CauHinh() {
  ...
  const addBusinessArea = () => {
    ...
    setEditConfig({ ...editConfig, businessAreas: [...editConfig.businessAreas, newArea] });
  };

  const removeBusinessArea = (id: string) => {
    setEditConfig({ ...editConfig, businessAreas: editConfig.businessAreas.filter(a => a.id !== id) });
  };

  const updateBusinessArea = (id: string, field: keyof BusinessArea, value: string) => {
    setEditConfig({
      ...editConfig,
      businessAreas: editConfig.businessAreas.map(a => a.id === id ? { ...a, [field]: value } : a),
    });
  };
  ...
  const addQuickLinkSection = () => {
    ...
    setEditConfig({ ...editConfig, quickLinks: [...editConfig.quickLinks, newSection] });
  };

  const removeQuickLinkSection = (id: string) => {
    setEditConfig({ ...editConfig, quickLinks: editConfig.quickLinks.filter(s => s.id !== id) });
  };

  const updateQuickLinkSection = (id: string, title: string) => {
    setEditConfig({
      ...editConfig,
      quickLinks: editConfig.quickLinks.map(s => s.id === id ? { ...s, title } : s),
    });
  };
  ...
  const removeLinkFromSection = (sectionId: string, linkIndex: number) => {
    setEditConfig({
      ...editConfig,
      quickLinks: editConfig.quickLinks.map(s =>
        s.id === sectionId ? { ...s, links: s.links.filter((_, i) => i !== linkIndex) } : s
      ),
    });
  };

  const updateLinkInSection = (sectionId: string, linkIndex: number, value: string) => {
    setEditConfig({
      ...editConfig,
      quickLinks: editConfig.quickLinks.map(s =>
        s.id === sectionId ? { ...s, links: s.links.map((l, i) => i === linkIndex ? value : l) } : s
      ),
    });
  };
  ...
  const addAnnouncement = () => {
    ...
    setEditConfig({ ...editConfig, announcements: [...editConfig.announcements, newAnnouncement] });
  };
  ...
  const updateAnnouncement = (id: string, field: keyof Announcement, value: string | boolean) => {
    setEditConfig({
      ...editConfig,
      announcements: editConfig.announcements.map(a => a.id === id ? { ...a, [field]: value } : a),
    });
  };
  ...
  return (
    <div className="-m-6 lg:-m-8">
        ...
        <div className="bg-gray-200 min-h-screen">
          <div className="max-w-7xl mx-auto py-6 px-4">
            ...
            {config.announcements.filter(a => a.isActive).length > 0 && (
              <EditableSection onEdit={() => setEditingSection("announcements")} label="Thông báo">
                <div className="p-4 space-y-3">
                  {config.announcements.filter(a => a.isActive).map((announcement) => {
                    const typeConfig = announcementTypes.find(t => t.value === announcement.type) || announcementTypes[0];
                    const TypeIcon = typeConfig.icon;
                    return (
                      <div key={announcement.id} className={`${typeConfig.bgColor} ${typeConfig.borderColor} border rounded-xl p-4 flex items-start gap-4`}>
                        <div className={`p-2 ${typeConfig.iconBg} rounded-lg shrink-0`}>
                          <TypeIcon className={typeConfig.textColor} size={20} />
                        </div>
                        <div>
                          <h4 className={`font-bold ${typeConfig.textColor}`}>{announcement.title}</h4>
            ...
            {/* Hero Section */}
            <EditableSection onEdit={() => setEditingSection("hero")} label="Banner">
            ...
            {/* About Us Section */}
            <EditableSection onEdit={() => setEditingSection("about")} label="Về chúng tôi">
            ...
            {/* Vision & Mission */}
            <EditableSection onEdit={() => setEditingSection("vision")} label="Tầm nhìn & Sứ mệnh">
            ...
            {/* Business Areas */}
            <EditableSection onEdit={() => setEditingSection("business")} label="Lĩnh vực hoạt động">
              <div className="relative bg-gradient-to-br from-blue-50 via-white to-blue-50 px-6 py-12">
                <div className="max-w-6xl mx-auto">
                  <div className="flex items-center gap-3 mb-8">
                    ...
                    <h2 className="text-3xl font-bold text-blue-600">LĨNH VỰC HOẠT ĐỘNG</h2>
                  </div>
            ...
            {/* Footer */}
            <EditableSection onEdit={() => setEditingSection("footer")} label="Footer">
      ...
      {/* Hero Edit Modal */}
      <EditModal isOpen={editingSection === "hero"} onClose={handleCancel} title="Chỉnh sửa Banner">
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Tiêu đề dòng 1 (màu cam)</label>
            <input
              type="text"
              value={editConfig.heroTitle1}
              onChange={(e) => setEditConfig({ ...editConfig, heroTitle1: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Tiêu đề dòng 2 (màu trắng)</label>
            <input
              type="text"
              value={editConfig.heroTitle2}
              onChange={(e) => setEditConfig({ ...editConfig, heroTitle2: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <div>
          ...
          <div className="flex gap-3 pt-4">
            <button onClick={handleCancel} className="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">Hủy</button>
      ...
      {/* About Edit Modal */}
      <EditModal isOpen={editingSection === "about"} onClose={handleCancel} title="Chỉnh sửa Về chúng tôi">
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              onChange={(e) => setEditConfig({ ...editConfig, aboutUs: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <div className="flex gap-3 pt-4">
      ...
      {/* Vision Edit Modal */}
      <EditModal isOpen={editingSection === "vision"} onClose={handleCancel} title="Chỉnh sửa Tầm nhìn & Sứ mệnh">
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              onChange={(e) => setEditConfig({ ...editConfig, vision: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <div>
            ...
            <textarea
              ...
              onChange={(e) => setEditConfig({ ...editConfig, mission: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <div className="flex gap-3 pt-4">
      ...
      <EditModal isOpen={editingSection === "business"} onClose={handleCancel} title="Chỉnh sửa Lĩnh vực hoạt động">
        <div className="space-y-4">
          {editConfig.businessAreas.map((area) => {
            const IconComponent = getIconComponent(area.icon);
            return (
              <div key={area.id} className={`bg-gradient-to-br ${area.color} rounded-xl p-4 text-white relative`}>
                <button
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      value={area.icon}
                      onChange={(e) => updateBusinessArea(area.id, "icon", e.target.value)}
                      className="w-full px-2 py-1.5 bg-white/20 border border-white/30 rounded text-white text-sm"
                    >
                      {iconOptions.map(opt => (
                  ...
                  <div className="col-span-2">
                    <label className="block text-xs text-white/70 mb-1">Mô tả</label>
                    <input
                      type="text"
                  ...
                  <div className="col-span-2">
                    <label className="block text-xs text-white/70 mb-1">Màu sắc</label>
                    <select
                      value={area.color}
                      onChange={(e) => updateBusinessArea(area.id, "color", e.target.value)}
                      className="w-full px-2 py-1.5 bg-white/20 border border-white/30 rounded text-white text-sm"
                    >
      ...
      {/* Footer Edit Modal */}
      <EditModal isOpen={editingSection === "footer"} onClose={handleCancel} title="Chỉnh sửa Footer">
        <div className="space-y-6">
          <div className="bg-gray-50 rounded-xl p-4 space-y-3">
            <h4 className="font-semibold text-gray-900">Thông tin công ty</h4>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-gray-600 mb-1">Tên công ty</label>
                <input
                  type="text"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditConfig({ ...editConfig, taxCode: e.target.value })}
                  className="w-full px-2 py-1.5 border border-gray-300 rounded text-sm"
                />
              </div>
              <div className="col-span-2">
              ...
              <div>
                <label className="block text-xs text-gray-600 mb-1">Người đại diện</label>
                <input
                  type="text"
                  value={editConfig.representative}
                  onChange={(e) => setEditConfig({ ...editConfig, representative: e.target.value })}
                  className="w-full px-2 py-1.5 border border-gray-300 rounded text-sm"
                />
              </div>
              <div>
                <label className="block text-xs text-gray-600 mb-1">Chức vụ</label>
                <input
                  type="text"
                  value={editConfig.position}
                  onChange={(e) => setEditConfig({ ...editConfig, position: e.target.value })}
                  className="w-full px-2 py-1.5 border border-gray-300 rounded text-sm"
                />
              </div>
            </div>
              ...
              <div key={section.id} className="bg-gray-50 rounded-xl p-3">
                <div className="flex items-center gap-2 mb-2">
                  <input
                    type="text"
                    value={section.title}
                    onChange={(e) => updateQuickLinkSection(section.id, e.target.value)}
                    className="flex-1 px-2 py-1 border border-gray-300 rounded text-sm font-medium"
                  />
                  <button onClick={() => removeQuickLinkSection(section.id)} className="p-1 text-red-500 hover:bg-red-50 rounded">
                    <Trash2 size={16} />
                  </button>
                </div>
                  ...
                  {section.links.map((link, linkIndex) => (
                    <div key={linkIndex} className="flex items-center gap-2">
                      <input
                        type="text"
                        value={link}
                        onChange={(e) => updateLinkInSection(section.id, linkIndex, e.target.value)}
                        className="flex-1 px-2 py-1 border border-gray-200 rounded text-sm"
                      />
                      <button onClick={() => removeLinkFromSection(section.id, linkIndex)} className="p-1 text-red-400 hover:text-red-600">
                        <X size={14} />
                      </button>
                    </div>
                  ))}
      ...
      <EditModal isOpen={editingSection === "announcements"} onClose={handleCancel} title="Quản lý Thông báo">
        <div className="space-y-4">
          <p className="text-gray-500 text-sm">Thêm các thông báo như chúc mừng Giáng sinh, Tết, lịch nghỉ lễ, v.v.</p>
          ...
          {editConfig.announcements.length === 0 ? (
            ...
            <div className="space-y-4">
              {editConfig.announcements.map((announcement) => {
                const typeConfig = announcementTypes.find(t => t.value === announcement.type) || announcementTypes[0];
                return (
                  <div key={announcement.id} className={`${typeConfig.bgColor} ${typeConfig.borderColor} border rounded-xl p-4 relative`}>
                    <button
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => updateAnnouncement(announcement.id, "type", e.target.value)}
